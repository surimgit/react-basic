...
// 브라우저 storage
// - 변수들을 사용하여 프로그램에서 메모리로 데이터를 관리하는 것이 아니라 브라우저 저장공간에서 관리하는 방법(반영구적)
// - storage: 브라우저(클라이언트의 데이터를 저장하고 관리)
// -          요청과는 무관한 특성
// -          쿠키와 비교했을 때 저장할 수 있는 용량이 큼
//            데이터를 key-value 형태로 저장
//            사용자가 삭제하지 않는한 형구적으로 유지할 수도 있음
//  - local storage: 브라우저를 닫아도 영구적으로 유지됨
//  - session storage: 현재 세션이 유지(브라우저가 켜져 있는)되는 동안 영구적으로 유지됨
...
//            보안이 우수하지만 서버의 메모리를 사용하여 부하를 일으킴
//            사용자의 로그인 정보를 유지하기 위해 많이 사용
export default function Storage() {

  const [storageKey, setStorageKey] = useState<string>('');
  const [storageValue, setStroageValue] = useState<string>('');

  // local storage 사용 방법
  // local storage 내장 객체 이용하여 local storage에 접근가능
