...
// route 경로에 있는 index.tsx의 root.render()안의 <App /> 컴포넌트를 <BrowerRouter> 컴포넌트로 감싸야함

// <BrowserRouter>: URL을 사용하여 브라우저 주소 표시줄에 현재 위치를 저장하고 탐색할 수 있게 해주는 컴포넌트

// <Routes> : <Route> 컴포넌트로 URL에 따른 컴포넌트를 렌더링 할 수 있도록 하는 컴포넌트
// <Route>: URL 패턴에 따라서 렌더링 하고자 하는 컴포넌트를 지정하는 컴포넌트

// - path: URL 패턴을 지정
// - element: 렌더링 할 컴포넌트를 지정
// - index: 현재 경로의 기본 라우터로 지정

function InteractionLayout() {
  // useLocation:
  // - 현재 경로에 대한 객체를 반환하는 react-router의 훅 함수
  // - pathname: 현재 path를 가지고 있는 속성
