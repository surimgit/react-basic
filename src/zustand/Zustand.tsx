...
// - Redux, MobX와 같은 타 상태 관리 패키지에 비해 가벼움 -> 빌드 시 패키징 속도가 굉장히 빠름
//   / 번들의 크기도 줄어둠

// npm install zustand

// zustand 이용한 전역 상태 선언 방법
// - zustand 패키지 내부의 create 함수로 store 생성
...
const useStore = create<Store>((set) => ({
  ...
  // set 함수의 매개변수로 전달한 콜백 함수는 변경된 상태(store)를 반환 ()
...
function D() {
  ...
  const { increaseState1, state1 } = useStore();
  const onChange = () => {
    increaseState1();
  }
