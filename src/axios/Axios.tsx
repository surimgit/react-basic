...
export default function Axios() {
  ...
  const onBasicGet = () => {
    ...
    // 각각의 http method 메서드는 then으로 response객체를 받는 콜백 함수를 전달
    // then은 request에 대해서 성공인 경우 수행
    // them은 response의 http 상태 코드 100~300을 처리
      .then((response) => {
        console.log(response.data);
      })
      // catch로 오류객체를 매개변수로 받는 콜백 함수를 전달
      // 응답이 존재하지 않는 경우 http 상태 코드 400~500을 처리
      // 응답이 존재하지 않는 경우도 처리
      .catch((error) => {
        // 응답이 존재할 경우 error 객체의 response 객체를 사용할 수 있음
        console.log(error.response.data);
        // 응답이 존재하지 않을 경우 response 속성이 undefined가 됨
  ...
  const onReqeustBodyClickHandler = () => {
    // axios의 post, patch, put 요청에 두 번째 매개변수로 requestBody를 지정할 수 있음
    // 객체타입이어야함
    // ! post, patch, put 메서드의 두 번째 매개변수는 반드시 requestBody임
    // ! request-body 없이 option만 지정하더라도 request-body에는 빈 객체가 있어야 함
    axios.post(
      'http://127.0.0.1:8080/request-data/request-body',
      { name, age }
    ).then((response) => {
      const responseBody = response.data;
      setResponseBody(responseBody);
  ...
  return (
    <div>
      ...
      <div>
        ...
        <button onClick={onReqeustBodyClickHandler}>Request Body</button>
