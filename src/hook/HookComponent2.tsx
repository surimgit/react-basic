...
export default function HookComponent2() {
  ...
  // 이때 해당 요소 시작 태그의 ref 속성에 해당 참조변수를 지정해야함
  ...
  const onFisrtKeydownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
    const { key } = event;
    if(key !== 'Enter') return;
    ...
    // 특정 개체에서 속성 혹은 함수 호출을 실행하고자 할 때
    // 해당 객체가 null 혹은 undefined를 가질 수 있는 경우라면
    // 해당 객체에 ?연산자를 사용하여 존재할 때만 접근 및 호출할 수 있게함
  ...
  return (
    <div>
      ...
      <div style={ {margin:'40px', padding: '40px', border: '1px solid gray'} }>
        <input id='first-input' onKeyDown={onFisrtKeydownHandler}/>
