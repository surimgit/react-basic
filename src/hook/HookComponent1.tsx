...
// Hook 함수:
// - react에서 컴포넌트의 생명주기와 및 상태에 따라 특정 작업을 수행하도록 하는 특별한 함수
// - use로 시작하는 함수

// Hook 함수의 종류
// - useState: 컴포넌트의 상태 관리를 가능하게 함, 상태 생성, 업데이트, 상태 접근)

// - useEffect: 컴포넌트의 생명주기에 따라 
// 특정 작업을 수행할 수 있도록 하는 함수(mount, update, unmount에 해당하는 생명주기에 따라 실행)
...
// - useReducer: 상태와 관련된 로직을 관리하는 함수
// (상태 업데이트 함수를 컴포넌트 외부로 옮길 수 있도록하여 코드 관리를 수월하게 도와줌)

// - useCallback: 메모리 상에서 함수를 재호출 할 수 있도록 하는 함수
// 렌더링 혹은 이벤트 없이 특정 함수를 호출할 때

// - useMemo: 메모리 내에 함수의 결과를 저장하여 함수 재호출하지 않고 결과를 사용할 수 있도록 하는함수
...
export default function HookComponent1() {
  ...
  // update 단계에서 특정 함수를 호출
  useEffect(() => {
    console.log('state값이 변경됨')
    // 스코프할 상태 배열에 지정된 상태를 해당 effect 내에서 변경하면 무한호출됨
  }, [state]);
